...
def pair_wise_score(pred_imgs, gt_imgs, metric, is_sucess):
    ...
    for idx, pred in enumerate(pred_imgs):
        ...
        if len(rest) == 0:  # Handle empty `rest`
            corrects.append(0)  # Default value if no comparison is possible
            continue
        count = 0
        for comp in rest:
            ...
            if is_sucess(gt_score, comp_score):
                count += 1
        corrects.append(count / len(rest))  # Safe division since `len(rest) > 0`
    return corrects
